...
export function CsvImportForm({ accountId }: { accountId: string }) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-4">
        <label className="mb-2 block text-xs text-neutral-500">CSV file</label>
        <input
          type="file"
          accept=".csv,text/csv"
          onChange={(e) => {
      ...
      {parsed && (
        <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-4">
          ...
          {parsed.errors.length > 0 && (
            <ul className="mb-3 max-h-40 list-disc space-y-1 overflow-y-auto pl-5 text-xs text-red-400">
